...
class event extends App{
	...
	function past(){
		...
		$this->log('surf', 'event');
		$getEvent = $this->newsHelper->pastEvent(false,true);
		...
		if ($getEvent){
			...
			foreach ($getEvent as $key => $val){
				...
				if ($loop<=3){
					...
					$dataEvent[$count][$index][$subindex] = $val;
			...
			$this->View->assign('dataEvent',$dataEvent);
		}
	...
	function pastDevel(){
		...
		return $this->View->toString(TEMPLATE_DOMAIN_WEB .'test/past-event-devel.html');
