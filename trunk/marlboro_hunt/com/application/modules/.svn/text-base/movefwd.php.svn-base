...
class movefwd extends App{
	...
	function beforeFilter(){
		...
		$this->imageType = array('image/jpeg','image/png','image/jpg','image/pjpeg','image/gif');
	...
	function uploadDataChallenge()
	{
		...
		if ($this->_p('token')){
			...
			if ($this->_p('embedLink')){
				$filename = "Embed Link Youtube";
				$saveChallenge = $this->contentHelper->saveChallenge($bucketid,$taskid,$filename,3);
				if ($saveChallenge){
					print json_encode(array('status'=>true, 'msg'=>'Embed link video success', 'cid'=>$bucketid));
				}else{
					print json_encode(array('status'=>false, 'msg'=>'not store link video'));
				}
				exit;
			}
			...
			if (isset($_FILES['dataChallenge']))
			{
				...
				if (isset($_FILES['dataChallenge']) && $_FILES['dataChallenge']['name']!=NULL) {
					...
					print json_encode(array('status'=>false, 'msg'=>'file does not defined'));
				}
			}else{
				print json_encode(array('status'=>false, 'msg'=>'file does not exist'));
			}
		}
	...
	function getEmbed()
	{
		if ($this->_p('uri')){
			...
			$uri = strip_tags($this->_p('uri'));
			$getEmbedVideo = $this->contentHelper->getVideoEmbededUrl(array('url'=>$uri));
			if ($getEmbedVideo){
				print json_encode(array('status'=>true, 'res'=>$getEmbedVideo));
