...
class minicooper extends Admin{
	...
	function giveletter($email=false,$code=false){
		...
		//1462
		$tasklist['minicooper'] = 1500; /* report ongorund purpose change to its content id if already fix content task */
		// $tasklist['minicooper'] = 1462;
		$tasklist['stranger'] = 1464;
		$tasklist['marlboros'] = 1465;
		$tasklist['onground'] = 1500;
	...
	function savetoinventory($uid=false,$code=false,$campaign='minicooper'){
		...
		global $LOCALE;
					...
					$date = date('Y-m-d H:i:s');
					$point = 2;
					$sql = "INSERT INTO  tbl_code_inventory (userid, codeid, codepublicityid, n_status, histories,datetimes,point)
							VALUES ({$uid}, {$code['codeid']}, {$code['id']}, 0, 'get from event {$campaign}','{$date}','{$point}')";
					$res = $this->query($sql);
