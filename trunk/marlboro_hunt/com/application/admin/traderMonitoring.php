...
class traderMonitoring extends Admin{
	...
	function home(){
		...
		/* list article */
		$sql = "SELECT tct.id, sm.name, tct.activity, tct.sourceCode, tct.targetCode, tct.otheruserid, tct.datetime, tct.n_status, tct.activity 
				FROM tbl_code_trade tct
				LEFT JOIN social_member sm ON tct.userid = sm.id
	...
	function gettradereport(){
		...
		$filename = "trade_report".date('Ymd_gia').".xls";
		...
		echo "<tr>";
			...
			echo "<th class='head1'>User Name</th>";
			echo "<th class='head1'>Source Letter</th>";
			echo "<th class='head1'>Target Letter</th>";
			echo "<th class='head1'>Status</th>";
			echo "<th class='head1'>Date Time</th>";
		echo "</tr>";
		foreach ($resReport as $key => $val){			
			echo "<tr>";
				...
				/* Source Code */
					if($val['sourceCode'] == 1) $sourceStatus = 'D';
					if($val['sourceCode'] == 2) $sourceStatus = 'O';
					if($val['sourceCode'] == 3) $sourceStatus = 'N';
					if($val['sourceCode'] == 4) $sourceStatus = 'T';
					if($val['sourceCode'] == 5) $sourceStatus = 'B1';
					if($val['sourceCode'] == 6) $sourceStatus = 'E1';
					if($val['sourceCode'] == 7) $sourceStatus = 'A1';
					if($val['sourceCode'] == 8) $sourceStatus = 'M';
					if($val['sourceCode'] == 9) $sourceStatus = 'A2';
					if($val['sourceCode'] == 10) $sourceStatus = 'Y';
					if($val['sourceCode'] == 11) $sourceStatus = 'B2';
					if($val['sourceCode'] == 12) $sourceStatus = 'E2';					
				echo "<td>$sourceStatus</td>";			
				
				/* targetCode */
					if($val['targetCode'] == 1) $targetStatus = 'D';
					if($val['targetCode'] == 2) $targetStatus = 'O';
					if($val['targetCode'] == 3) $targetStatus = 'N';
					if($val['targetCode'] == 4) $targetStatus = 'T';
					if($val['targetCode'] == 5) $targetStatus = 'B1';
					if($val['targetCode'] == 6) $targetStatus = 'E1';
					if($val['targetCode'] == 7) $targetStatus = 'A1';
					if($val['targetCode'] == 8) $targetStatus = 'M';
					if($val['targetCode'] == 9) $targetStatus = 'A2';
					if($val['targetCode'] == 10) $targetStatus = 'Y';
					if($val['targetCode'] == 11) $targetStatus = 'B2';
					if($val['targetCode'] == 12) $targetStatus = 'E2';		
				echo "<td>$targetStatus</td>";	

				/* Other ID */
					if(($val['n_status'] == 0) and ($val['activity']==1)) $otherStatus = 'Open Trade';	
					if(($val['n_status'] == 1) and ($val['activity']==1)) $otherStatus = 'Trade Pending';	
					if(($val['n_status'] == 2) and ($val['activity']==1)) $otherStatus = 'Success Trade';	
					if(($val['n_status'] == 2) and ($val['activity']==0)) $otherStatus = 'Cancel Trade';	
					if(($val['n_status'] == 0) and ($val['activity']==0)) $otherStatus = 'Cancel Trade';	
					
				echo "<td>$otherStatus</td>";
	...
	function downloadreport(){
		...
		// $sql = "SELECT tct.id, sm.name, tct.activity, tct.sourceCode, tct.targetCode, tct.otheruserid, tct.datetime 
				...
				// LEFT JOIN social_member sm ON tct.userid = sm.id
				// LEFT JOIN tbl_code_detail tcd ON tct.sourceCode = tcd.id WHERE sm.name IS NOT NULL {$filter}
				// GROUP BY tct.datetime ORDER BY tct.datetime DESC";
