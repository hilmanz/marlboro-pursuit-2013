...
class merchandiseStock extends Admin{
	...
	function add(){	
		...
		$data['need_point'] 		= $this->_p('need_point');		
		...
		if($this->_p('simpan')){		
			...
			$sql = "INSERT INTO {$this->dbclass}_merchandise (name,detail,stock,letter,postdate,n_status,merchandise_type, need_point) 
			VALUES ('{$name}','{$detail}','{$stock}','{$letter}','{$postdate}','{$n_status}','{$merchandise_type}','{$need_point}')";
			$this->query($sql);
			// pr($sql);exit;
	...
	function edit(){
		...
		if(! $this->_p('simpan')){
			...
			$point 		= $this->_p('need_point');
			$postdate 		= $this->_p('postdate');
			
			if($this->roler['approver']) $status = $this->_p('n_status');
			else $status 	 = 0;
			
			if($this->roler['approver']) $merch = $this->_p('merchandise_type');
			else $merch 	 = 0;
