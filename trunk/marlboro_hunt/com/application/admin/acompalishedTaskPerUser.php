...
class acompalishedTaskPerUser extends Admin{
	...
	function home(){
		...
		$sql = "SELECT count(*) total 
				FROM my_task mt LEFT JOIN social_member sm ON mt.userid = sm.id
				LEFT JOIN {$this->dbclass}_news_content con ON mt.taskid = con.id
				WHERE sm.name IS NOT NULL AND con.title IS NOT NULL 
				{$filter}";
		// pr($sql);		
		...
		// pr($total);
		/* list article */
		$sql = "SELECT sm.name, sm.last_name, con.title, mt.userid, mt.taskdate,mt.n_status 
				FROM my_task mt LEFT JOIN social_member sm ON mt.userid = sm.id
				LEFT JOIN {$this->dbclass}_news_content con ON mt.taskid = con.id
				WHERE sm.name IS NOT NULL AND con.title IS NOT NULL {$filter}
				ORDER BY mt.taskdate DESC LIMIT {$start},{$this->total_per_page}";
		// pr($sql);
	...
	function getAcompalishedReport(){
		
		$filename = "acompalished_task_report".date('Ymd_gia').".xls";
		...
		echo "<tr>"; 	
			...
			echo "<th class='head1'>Task Date</th>";
		echo "</tr>";
