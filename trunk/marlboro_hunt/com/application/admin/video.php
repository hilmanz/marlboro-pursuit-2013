...
class video extends Admin{
	...
	function comment(){
		...
		/* Hitung banyak record data */
		$sql ="FROM {$this->dbclass}_news_content_comment cm
	...
	function createFiles($last_id=null){
				global $CONFIG;
				if($last_id==null) return false;
				if ($_FILES['thefiles']['name']!=NULL) {
					
					include_once '../../engines/Utility/phpthumb/ThumbLib.inc.php';
					list($file_name,$ext) = explode('.',$_FILES['thefiles']['name']);
					$files = md5($_FILES['thefiles']['name'].rand(1000,9999)).".".$ext;
					 
			
					if(move_uploaded_file($_FILES['thefiles']['tmp_name'],"{$CONFIG['LOCAL_PUBLIC_ASSET']}{$this->folder}/{$files}")){
						$this->inputFiles($last_id,$files);
					}
	...
	function inputFiles($id,$file){
		$this->query("UPDATE {$this->dbclass}_news_content SET file='{$file}' WHERE id={$id}");
	}
