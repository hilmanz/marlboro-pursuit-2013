...
class emailBlast extends Admin{
	var $category;
	var $type;
	function __construct(){
		parent::__construct();

		$this->folder =  'emailBlast';
	...
	function home(){
		...
		/* LIST USER MEMBER */
		$sql = "(
					SELECT mm.message message, datetime
					FROM my_message AS mm
					WHERE mm.message IS NOT NULL
					GROUP BY mm.id
				)
				UNION(
					SELECT description message, date_time datetime
					FROM gm_activity_log
					WHERE ACTION = 'inputtask'
					AND n_status =1
				)
		...
		if(isset($tes)){
			...
			if($this->_p('simpan')){
				...
				if ($notif){
					...
					$sql = "INSERT INTO gm_activity_log (admin_id, date_ts, date_time, action, Description, n_status) VALUES (1,'{$timeStamp}','{$date}','inputtask','{$message}',1 )";
					$res = $this->query($sql);
					// pr($sql);
				}
